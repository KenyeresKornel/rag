package com.example.arxivrag.vector.milvus;
...
/**
 * Implementation of {@link VectorStoreGateway} for Milvus Standalone using Spring AI's {@link VectorStore}.
 */
@Component
@Profile("store-milvus")
public class MilvusVectorStoreGateway implements VectorStoreGateway {
    ...
    @Autowired
    public MilvusVectorStoreGateway(VectorStore vectorStore) {
    ...
    public void save(List<RetrievalDocument> documents) {
        ...
        log.info("Saving {} documents to Milvus Vector Store...", documents.size());
    ...
    public void save(List<RetrievalDocument> documents, List<float[]> embeddings) {
        throw new UnsupportedOperationException("User-provided embeddings are not supported by the current Spring AI version on Milvus.");
    }
    ...
    public void deleteByPaperIds(List<String> paperIds) {
        ...
        log.info("Deleting documents from Milvus associated with paper IDs: {}", paperIds);
